  ...
  return (
    <div className="page">
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 14px', fontSize: 13 }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--accent)' }}>{totalHours.toFixed(1)}h</span>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="card">
          ...
          {activity.meetings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text2)', fontSize: 13 }}>No meetings today</div>
          ) : (
              ...
              {activity.meetings.map((m, i) => (
                <div key={i} style={{ background: 'var(--bg3)', borderRadius: 8, padding: '10px 12px' }}>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                    ...
                    <button onClick={() => removeMeeting(i)} style={{ background: 'none', color: 'var(--text3)' }}><X size={13} /></button>
                  </div>
        ...
        <div className="card">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontWeight: 600, fontSize: 14 }}>Tasks Worked On</span>
          ...
          {activity.tasks.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '20px', color: 'var(--text2)', fontSize: 13 }}>No tasks logged yet</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {activity.tasks.map((t, i) => (
                <div key={i} style={{ background: 'var(--bg3)', borderRadius: 8, padding: '10px 12px' }}>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                    ...
                    <button onClick={() => removeTask(i)} style={{ background: 'none', color: 'var(--text3)' }}><X size={13} /></button>
                  </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginTop: 16 }}>
        <div className="card">
          <label style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 10 }}>Notes</label>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 16, color: 'var(--text2)' }}>How was your day?</div>
          <div style={{ fontSize: 36, marginBottom: 12 }}>{MOODS[activity.mood - 1]}</div>
          <div style={{ display: 'flex', gap: 8 }}>
            {MOODS.map((m, i) => (
              <button key={i} onClick={() => setActivity(p => ({ ...p, mood: i + 1 }))}
                style={{ fontSize: 22, background: 'none', opacity: activity.mood === i + 1 ? 1 : 0.3, transform: activity.mood === i + 1 ? 'scale(1.2)' : 'scale(1)', transition: 'all 0.15s' }}>
                {m}
              </button>
            ))}
          </div>
